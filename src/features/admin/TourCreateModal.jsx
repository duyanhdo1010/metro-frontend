...
function TourCreateModal({ onClose }) {
  // handle imageCover
  ...
  // handle date picker
  const [startDates, setStartDates] = useState([addDays(new Date(), 1)]);
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)}>
      <FormRow>
        <RowWrapper>
          {imageCover ? <StyledPreviewImage alt="tour imageCover" src={imageCover} /> : null}
          <div>
            <FileLabel htmlFor="imageCover">Thêm ảnh bìa tour</FileLabel>
            <Input
              id="imageCover"
              {...register('imageCover')}
        ...
        <ImagesContainer>
          {images.map((preview, index) => (
            <ImageContainer key={index}>
              <StyledPreviewImage key={index} src={preview} alt={`Preview ${index + 1}`} />
